...
import { speakWithZena } from "@/lib/tts";
...
export default function ZenaAvatar({ textToSpeak = "" }: ZenaAvatarProps) {
  ...
  useEffect(() => {
    if (textToSpeak) {
      ...
      setEmotion(detected);
      setIsSpeaking(true);
      speakWithZena(textToSpeak).finally(() => {
        setIsSpeaking(false);
      });
    }
  }, [textToSpeak]);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: isSpeaking ? 1.5 : 0.3, repeat: isSpeaking ? Infinity : 0, ease: "easeInOut" }}
      >
        <video
        ...
        />
        <img src={zenaImage} alt="ZÉNA" className="hidden w-full h-full object-cover rounded-full" />
      </motion.div>

      {/* Nom et tagline */}
      <motion.div className="mt-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }}>
